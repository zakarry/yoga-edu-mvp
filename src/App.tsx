      ...
      <main className="main-shell">
        {page === 'home' && (
          <div className="page-shell top-renewal">
            {/* === LOGGED-IN HERO: AI先生 + 今日のヨガ as primary CTAs === */}
            {auth.user ? (
              <>
                <section className="hero-panel federation-hero top-hero top-hero-logged-in">
                  <div className="hero-copy-block">
                    <h1>今日のヨガを始めましょう</h1>
                    <p className="hero-subcopy">AI先生があなたに合う今日のプログラムを一緒に考えます。</p>
                  </div>
                  <div className="hero-actions wrap top-hero-actions top-hero-actions-logged-in">
                    <button className="primary-button hero-primary-cta hero-cta-ai-teacher" onClick={() => moveTo('ai-teacher')}>
                      <span className="hero-cta-icon">🧘</span>
                      <span>AI先生に会う</span>
                    </button>
                    <button className="primary-button hero-primary-cta hero-cta-today-yoga" onClick={() => moveToAITeacher(10)}>
                      <span className="hero-cta-icon">✨</span>
                      <span>今日のヨガ</span>
                    </button>
                ...
                {/* AI Teacher feature card — prominent */}
                ...
                <section className="panel top-daily-panel">
                  ...
                  <p className="top-daily-status">実践履歴はmyYOGAカルテに記録されます</p>
                </section>
                ...
                <section className="panel top-intent-panel">
                  ...
                  <div className="top-intent-grid">
                    ...
                    <button className="top-intent-card" onClick={() => moveTo('my-page')}>
                      <span className="top-intent-icon">📋</span>
                      <strong>記録を見たい</strong>
                      <span className="top-intent-desc">myYOGAカルテで履歴を確認</span>
                      <span className="top-intent-cta">myYOGAカルテ →</span>
                    </button>
                  </div>
                </section>
              </>
              ...
              <>
                ...
                {/* Yoga AIでできること */}
                <section className="panel top-capabilities-panel">
                  <div className="section-inline-header">
                    <h3>Yoga AIでできること</h3>
            ...
            {/* === SHARED SECTIONS (both logged-in and logged-out) === */}
        ...
        {page === 'results' && diagnosisResult && (
          <ResultPage
            ...
            onOpenBreathworkGuide={(entryId) => { setBreathworkEntryId(entryId); moveTo('breathwork-practice'); }}
        ...
        {page === 'terms' && (
          <StaticInfoPage
            eyebrow="Terms"
            title="利用規約"
            intro="本利用規約は、一般社団法人 全日本ヨガ連盟（以下「当会」といいます）が運営するYOGAI.net（以下「本サービス」といいます）の利用条件を定めるものです。本サービスをご利用いただく前に、本規約をよくお読みください。"
            sections={[
              {
                heading: '第1条（サービスの目的）',
                body: '本サービスは、ヨガの診断、学習、実践支援、指導者・スクール・イベント情報の提供等を目的とする情報提供サービスです。AIを活用した診断や提案、実践プログラムの提供、myYOGAカルテ等の保存機能を含みますが、これらは参考情報として提供されるものであり、医療上の診断、治療、処方その他の医療行為を目的とするものではありません。',
              },
              {
                heading: '第2条（アカウント）',
                body: '本サービスの一部の機能を利用するには、メールアドレス、Googleアカウント、またはLINEアカウントによる会員登録が必要です。会員は、登録情報について正確かつ最新の内容を維持する責任を負います。アカウントの管理不十分による損害について、当会は責任を負いません。',
              },
              {
                heading: '第2条の2（Yoga AI会員）',
                body: '「Yoga AI会員」とは、本規約およびプライバシーポリシーに同意し、YOGAI.netの会員登録を完了した者をいいます。Yoga AI会員は、一般社団法人 全日本ヨガ連盟が運営するデジタルサービス上の会員区分です。Yoga AI会員であることのみをもって、当連盟の他の会員区分、資格、議決権その他の権利が当然に付与されるものではありません。',
              },
              {
                heading: '第3条（AI機能について）',
                body: '本サービスはAI（人工知能）を利用して診断、提案、実践プログラム、チャット回答等を生成します。AIによる回答・提案・診断結果等には誤りが含まれる可能性があります。AI先生は、人間のヨガ指導者、医師その他の専門家を代替するものではありません。AIの回答に基づいてご自身の判断で行動された結果について、当会は責任を負いません。',
              },
              {
                heading: '第4条（健康・安全）',
                body: '本サービスは医療上の診断、治療、処方その他の医療行為を目的とするものではありません。痛み、強い違和感、めまい、息苦しさその他の異常を感じた場合は直ちに実践を中止してください。医師から運動制限を受けている場合、妊娠中・産後など特別な配慮が必要な場合は、事前に医師等の専門家にご相談ください。体調に不安がある場合は、無理をせず専門家へご相談ください。',
              },
              {
                heading: '第5条（myYOGAカルテ等の保存機能）',
                body: '本サービスでは、診断結果、実践履歴、AI先生の記憶等をクラウドに保存する機能を提供します。保存は利用者のプライバシー設定に基づき、利用者の同意を得た場合のみ行われます。保存された情報は、利用者本人が設定画面で管理できます。保存機能を利用するには会員登録およびログインが必要です。',
              },
              {
                heading: '第6条（先生、スクール、イベント等の情報）',
                body: '本サービスに掲載される先生、スクール、イベント、ヨガクラブ等の情報は、情報提供を目的とするものです。掲載情報の正確性について当会は保証しません。掲載情報を利用される際は、利用者ご自身の判断で確認してください。',
              },
              {
                heading: '第7条（禁止事項）',
                body: '本サービスの利用にあたり、以下の行為を禁止します。法令違反、他者の権利侵害、虚偽情報の登録、迷惑行為、不正アクセス、本サービスの運営妨害、その他当会が不適切と判断する行為。',
              },
              {
                heading: '第8条（知的財産権）',
                body: '本サービスに掲載されるコンテンツ（文章、画像、音声等）の知的財産権は、当会または正当な権利者に帰属します。利用者は、本サービスのコンテンツを複製、転載、配信等することはできません。利用者が入力した情報については、本サービスの提供目的の範囲内で当会が利用できるものとします。',
              },
              {
                heading: '第9条（サービスの変更・停止）',
                body: '当会は、本サービスの内容を変更、追加、停止する場合があります。これらにより利用者に生じた損害について、当会は責任を負いません。',
              },
              {
                heading: '第10条（免責）',
                body: '本サービスは情報提供サービスであり、利用者の自己判断のもとでご利用いただきます。本サービスの利用により生じた損害について、当会は責任を負いません。AIの回答、診断結果、実践プログラム等は参考情報であり、その正確性、有用性、安全性を保証するものではありません。',
              },
              {
                heading: '第11条（退会・アカウント削除）',
                body: '利用者は、いつでも本サービスの利用を終了できます。退会・アカウント削除に関する手続きは、別途当会が定める方法で行います。退会後、保存されたデータは当会の定める期間後に削除される場合があります。',
              },
              {
                heading: '第12条（未成年者）',
                body: '未成年者が本サービスを利用する場合は、年齢、サービス内容、取り扱う情報に応じて、保護者等の同意を求める場合があります。保護者の同意が得られない場合は、本サービスの利用を制限する場合があります。',
              },
              {
                heading: '第13条（規約の変更）',
                body: '当会は、本規約を変更する場合があります。変更後の規約は、本サービスに掲載した時点から効力を生じるものとします。重要な変更については、本サービス上で通知します。',
              },
              {
                heading: '第14条（準拠法・管轄）',
                body: '本規約は、日本法に準拠します。本規約に関する紛争は、当会の本部所在地を管轄する裁判所を第一審の専属的合意管轄裁判所とします。',
              },
            ]}
          />
        )}
        {page === 'privacy' && (
          <StaticInfoPage
            eyebrow="Privacy"
            title="プライバシーポリシー"
            intro="一般社団法人 全日本ヨガ連盟（以下「当会」といいます）は、YOGAI.net（以下「本サービス」といいます）の利用者から取得する情報の取扱いについて、以下の通り定めます。"
            sections={[
              {
                heading: '1. 取得する情報',
                body: '本サービスでは、以下の情報を取得する場合があります。\n・メールアドレス等の認証情報\n・Google認証により取得する情報（Googleアカウントの識別子等）\n・LINE認証により取得する識別情報・表示名等\n・プロフィール情報（表示名、ロール、エリア、希望言語等）\n・AI診断の入力内容および結果\n・身体状態、痛み、けが、妊娠、既往症、運動制限等、利用者が入力する健康関連情報\n・ヨガ、呼吸法、瞑想等の実践履歴\n・AI先生の記憶として保存を許可した情報（好み、目標、懸念事項等）\n・LINE上でYoga AIとやり取りしたメッセージ内容（安全上問題のある内容を除く）\n・利用者の端末内ストレージ（localStorage）に保存する情報（実践記録、AI先生の設定、診断履歴等）',
              },
              {
                heading: '2. 利用目的',
                body: '取得した情報は、以下の目的で利用します。\n・本人認証・会員登録の管理\n・本サービスの提供（AI診断、AI先生による提案、myYOGAカルテ、実践履歴の保存・表示）\n・パーソナライズ（利用者の好みや傾向に合わせた提案）\n・問い合わせ対応\n・安全性の確保（安全性情報の検出、実践の制限、専門家への相談案内等）\n・サービスの改善・開発\n・不正利用の防止',
              },
              {
                heading: '3. 利用する外部サービス',
                body: '本サービスは、以下の外部サービスを利用しています。各サービスの利用に伴い、利用者の情報がこれらの事業者に提供される場合があります。詳細は各サービス提供者のプライバシーポリシー等をご確認ください。\n・Supabase（認証・データベース・サーバー機能の提供）\n・Google（Googleアカウントを利用した認証）\n・LINE（LINEアカウントを利用した認証、LINEボットによる対話）\n・OpenAI（AIによる回答生成）',
              },
              {
                heading: '4. 情報の保存・安全管理',
                body: '取得した情報は、Supabaseが提供するクラウドデータベースに保存されます。健康関連情報（痛み、けが、妊娠、既往症等）のクラウド保存は、利用者がプライバシー設定で明示的に許可した場合のみ行われます。利用者の端末内ストレージ（localStorage）には、サービスの利便性向上、実践記録、AI先生の設定等のため、一部の情報が保存される場合があります。当会は、取得した情報の安全管理のために合理的な措置を講じます。',
              },
              {
                heading: '5. 本人による設定変更',
                body: '利用者は、myYOGAカルテの設定画面で、以下を自身で管理できます。\n・AI診断のクラウド保存のON/OFF\n・実践履歴のクラウド保存のON/OFF\n・AI先生の記憶のON/OFF\n・リアル先生との共有のON/OFF\n・身体状態等のセンシティブ情報のクラウド保存のON/OFF\n各設定はデフォルトでOFFとなっており、利用者の明示的な操作によりONにした場合のみ、該当する情報のクラウド保存が行われます。OFFにした場合、以降の新規保存は停止されます。',
              },
              {
                heading: '6. 開示、訂正、削除等の問い合わせ',
                body: '利用者は、当会に対し、自己の個人情報の開示、訂正、削除等を求めることができます。開示、訂正、削除等をご希望の場合は、当会のお問い合わせ窓口までご連絡ください。ただし、本サービス内の一部のデータ（診断履歴、実践履歴、AI先生の記憶）については、利用者自身がアプリ内で個別に削除できる機能を提供しています。',
              },
              {
                heading: '7. LINEとの連携について',
                body: 'LINEアカウントを利用して本サービスに会員登録した場合、LINEの識別情報と本サービスの会員情報が紐付けられます。LINE上でYoga AIとメッセージをやり取りした場合、メッセージ内容はAI処理および安全性確認のために利用されます。LINE友だち追加のみで本サービスに会員登録していない場合、LINE上のメッセージはYOGAI.net会員とは紐付けられません。詳細はLINE社のプライバシーポリシー等をご確認ください。',
              },
              {
                heading: '8. AI利用について',
                body: '本サービスは、OpenAIのAPIを利用してAIによる回答・提案を生成します。利用者の入力内容は、回答生成のためにAI処理に利用されます。AIの回答には誤りが含まれる可能性があり、医療上の診断や専門家の助言を代替するものではありません。',
              },
              {
                heading: '9. ポリシーの変更',
                body: '当会は、本プライバシーポリシーを変更する場合があります。変更後のポリシーは、本サービスに掲載した時点から効力を生じるものとします。重要な変更については、本サービス上で通知します。',
              },
              {
                heading: '10. お問い合わせ窓口',
                body: '本プライバシーポリシーに関するお問い合わせは、当会までご連絡ください。正式な問い合わせ先設定が必要です。',
              },
            ]}
          />
        )}
